...
function showFatal(msg) {
    const ids = ['kpiCards', 'timeChart', 'dayChart', 'hourlyChart', 'nightChart', 'eventChart'];
    ids.forEach(id => {
        const el = document.getElementById(id);
        if (!el) return;
        el.innerHTML = `<div class="loading"><div style="text-align:center;max-width:520px">
          <div style="font-weight:700;margin-bottom:.25rem">Dashboard can’t load</div>
          <div style="color:#a3a3a3;font-size:.95rem">${msg}</div>
        </div></div>`;
    });
}
...
async function load() {
    if (!window.Plotly) {
        showFatal('Plotly (the chart library) did not load. This usually happens when the CDN is blocked/slow. Try reloading, or connect to the internet, or replace Plotly with a locally hosted file.');
        return;
    }
    const [st, hist] = await Promise.all([f('stats'), f('historical')]);
    if (!st || !hist) {
        showFatal('The API did not respond. Make sure the server is running on http://localhost:5005 and refresh.');
        return;
    }
