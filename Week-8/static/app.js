...
function cVeh(data) {
    Plotly.newPlot('vehChart', [{
        ...
        type: 'scatter', mode: 'markers', marker: { size: 2, color: data.map(d => d.is_anomaly ? 'rgb(239,68,68)' : 'rgba(245,87,166,.25)') }
    }],
...
function cHourly(data) {
    ...
    Plotly.newPlot('hourlyChart', [{ x: hrs, y: vals, type: 'bar', marker: { color: vals.map(v => v > 15 ? AC : 'rgba(245,87,166,.35)') } }], { ...L, xaxis: { ...L.xaxis, title: 'Hour', tickmode: 'linear', dtick: 2 }, yaxis: { ...L.yaxis, title: 'Avg kWh' } }, C);
}
