...
function cPie(d) {
    ...
    const el = document.getElementById('pieChart'); if (el) el.innerHTML = '';
...
function cCluster(d) {
    ...
    const el = document.getElementById('clusterChart'); if (el) el.innerHTML = '';
...
function cTime(data) {
    if (!data || !data[0] || !data[0].cluster_label) return;
    const el = document.getElementById('timeChart'); if (el) el.innerHTML = '';
...
function cHourly(data) {
    if (!data || !data[0] || !data[0].cluster_label) return;
    const el = document.getElementById('hourlyChart'); if (el) el.innerHTML = '';
    const labels = [...new Set(data.map(d => d.cluster_label))];
    const traces = labels.map(lbl => {
...
function cWknd(data) {
    ...
    const el = document.getElementById('wkndChart'); if (el) el.innerHTML = '';
